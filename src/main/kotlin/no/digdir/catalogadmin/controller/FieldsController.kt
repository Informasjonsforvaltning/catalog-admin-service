package no.digdir.catalogadmin.controller

import no.digdir.catalogadmin.model.EditableFields
import no.digdir.catalogadmin.model.Field
import no.digdir.catalogadmin.model.FieldToBeCreated
import no.digdir.catalogadmin.model.Fields
import no.digdir.catalogadmin.model.JsonPatchOperation
import no.digdir.catalogadmin.security.EndpointPermissions
import no.digdir.catalogadmin.service.FieldsService
...
class FieldsController(private val endpointPermissions: EndpointPermissions, private val fieldsService: FieldsService) {
    ...
    ): ResponseEntity<EditableFields> = if (endpointPermissions.hasOrgAdminPermission(jwt, catalogId)) {
    ...
    ): ResponseEntity<Field> = if (endpointPermissions.hasOrgReadPermission(jwt, catalogId)) {
        fieldsService
    ...
    fun patchInternalField(
        ...
        @PathVariable fieldId: String,
        @RequestBody patchOperations: List<JsonPatchOperation>,
    ): ResponseEntity<Field> = if (endpointPermissions.hasOrgAdminPermission(jwt, catalogId)) {
        fieldsService
